...
public class Main extends GameBase implements PlayerDieListener, PlayerWinListener, ScreenTransitionListener, PokemonChatController.ChatActionListener, DialogueTriggerListener {
	...
	public void update(float tslf) {
		...
		if (currentLevel.getShopNPCs().size() > 0) {
			currentShopGUI = currentLevel.getShopNPCs().getFirst();
		} else {
	...
	public void draw(Graphics g) {
		...
		if (currentLevel.getShopNPCs().size() > 0) {
			currentShopNPC = currentLevel.getShopNPCs().getFirst();
		} else {
	...
	public void loadGame() {
		try (ObjectInputStream in = new ObjectInputStream(
			...
			for (CountrySaveData csd : saveData.countryEconomies) {
				...
				countryList.getLast().setTime(csd.time);
				countryList.getLast().setPop(csd.population);
				countryList.getLast().setFL(csd.frontLine);
				countryList.getLast().setInWar(csd.isInWar);
				countryList.getLast().setCountryNews(csd.countrynews);
				countryList.getLast().setTax(csd.taxRate);
				countryList.getLast().setSpending(csd.spending);
